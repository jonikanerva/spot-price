...
export interface Provenance {
  ...
  /**
   * MEASURED out-of-sample coverage — recorded here for transparency even when
   * the shipped `observedCoverage` field is null because the band is dark.
   */
 ...
 *
 * Regenerated by the offline dev script (\`pnpm tsx tools/regenerate-bands.ts\`),
 ...
 * fetches live history directly. The production vintage table is too large to
 * round-trip through a single JSON string: `JSON.stringify` overflows the max
 * string length. `--db` is still an offline MANUAL chore, never a background
 * job.
 */
