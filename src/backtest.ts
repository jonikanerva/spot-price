/**
 * OFFLINE / DEV-ONLY rolling-origin backtest for the FI price forecast.
 *
 * IMPORTANT: this module must NOT be imported by any route, `index.ts`, or
 * `app.ts`. It is not on the shipped request path and adds NO background job
 * (`STACK.md §9`). Because the production bundle's only tsup entry is
 * `src/index.ts` and nothing in that graph imports this file, it never reaches
 * `dist/`. It is exercised two ways:
 *   - as a vitest (`backtest.test.ts`, the leakage guard + metric sanity);
 *   - as a `tsx`-runnable script over JSON fixtures:
 *       pnpm tsx src/backtest.ts --data tools/backtest-data
 *
 ...
 * Scoring is against the realised (uncensored) prices. Metrics are MAE / rMAE /
 * sMAPE versus two naive baselines the model must beat to justify itself:
 * same-quarter-last-week and last-published-day.
 *
 * Strictly pure below `main`: the exported helpers take no I/O. `main` reads
 * fixtures via `node:fs` and `process.argv` only (no `process.env`, no network,
 * no DB).
 */
...
const main = (): void => {
  ...
  const s = runBacktest(data);

  // Touch the bucketing helpers so the dev report can note Fingrid coverage —
  // also keeps the imports meaningful for anyone extending the report.
  ...
  console.log(`  leakage-free:        ${String(s.leakFree)}`);
  console.log(
    `  wind/cons coverage:  ${String(wind.size)} / ${String(cons.size)} quarters`,
  );
  console.log(`  model MAE  (c/kWh):  ${fmt(s.modelMae)}`);
  console.log(`  model sMAPE:         ${fmt(s.modelSmape)}`);
  for (const name of BASELINES) {
    console.log(
      `  ${name.padEnd(20)}MAE ${fmt(s.baselineMae[name])}  rMAE ${fmt(
        s.rMae[name],
      )}`,
    );
  }
...
// Run only when invoked as a script (tsx src/backtest.ts), never on import.
